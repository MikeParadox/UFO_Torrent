...
constexpr int block_size{16384};   // 2^14
constexpr int max_pending_time{5}; // 5 sec
constexpr int progress_bar_width{40};
constexpr int progress_display_interval{1}; // 0.5 sec
...
std::vector<Piece*> PieceManager::initiatePieces()
{
    ...
    for (int i = 0; i < totalPieces; i++)
    {
        ...
        if (i == totalPieces - 1)
        {
            ...
            blockCount =
                std::max(static_cast<int>(ceil(remLength / block_size)), 1);
        }
